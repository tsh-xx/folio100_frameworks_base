...
EventHub::device_t::device_t(int32_t _id, const char* _path, const char* name)
    : id(_id), path(_path), name(true, name), classes(0)
...
bool EventHub::getKeyLayoutFileName(const struct input_id *id, const char *inputName,
                                   char *optionalName, char *keylayoutFilename,
                                   size_t len) {
    char tmpfn[80];
    bool fileAcessError;

    // replace all the spaces with underscores
    strcpy(tmpfn, inputName);
    ...
    strcpy(optionalName, inputName);

    // Check for keylayout file with input name.
    snprintf(keylayoutFilename, len, "%s/usr/keylayout/%s.kl", root, tmpfn);
    fileAcessError = access(keylayoutFilename, R_OK);
    if (!fileAcessError)
         return false;

    // If usb keyboard then use the default usb keyboard keylayout file.
    if (id->bustype == BUS_USB) {
          snprintf(keylayoutFilename, len,
                     "%s/usr/keylayout/%s", root, "usb_keyboard_102_en_us.kl");
          fileAcessError = access(keylayoutFilename, R_OK);
          if (!fileAcessError) {
               strcpy(optionalName, "usb_keyboard_102_en_us");
               return true;
          }
    }

    // Use the android default keylayout file if no keylayout file exist.
    // Android default keylayout file "qwerty.kl" exist by default.
    snprintf(keylayoutFilename, len,
                     "%s/usr/keylayout/%s", root, "qwerty.kl");
    strcpy(optionalName, "qwerty");
    return true;
}
// ----------------------------------------------------------------------------
...
int EventHub::open_device(const char *deviceName)
{
    ...
    if ((device->classes&CLASS_KEYBOARD) != 0) {
        ...
        char optionalname[sizeof(name)];

        defaultKeymap = getKeyLayoutFileName(&id, name, optionalname,
        ...
        property_set(propName, optionalname);
