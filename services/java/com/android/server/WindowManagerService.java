...
public class WindowManagerService extends IWindowManager.Stub
    ...
    public int START_CURSOR_X;
    public int START_CURSOR_Y;
    Object mCursor;
    Method mShowCursor;
    Method mMoveCursor;
    int mMlx = -1;
    ...
    private WindowManagerService(Context context, PowerManagerService pm,
        ...
        try {
            String path = "jar:file://system/framework/com.nvidia.graphics.jar!/";
            URL searchPaths[] = {new URL(path)};
            URLClassLoader cl = new URLClassLoader(searchPaths);
            Class cursorClass;

            cursorClass = cl.loadClass("com.nvidia.graphics.Cursor");
            mCursor = cursorClass.newInstance();
            Class clShow[] = new Class[1];
            Class clMove[] = new Class[5];
            clShow[0] = Boolean.TYPE;
            clMove[0] = Integer.TYPE;
            clMove[1] = Integer.TYPE;
            clMove[2] = Integer.TYPE;
            clMove[3] = Integer.TYPE;
            clMove[4] = Integer.TYPE;

            mShowCursor = cursorClass.getMethod("ShowCursor", clShow);
            mMoveCursor = cursorClass.getMethod("MoveCursor", clMove);
        } catch (Exception e) {
            mShowCursor = null;
            mMoveCursor = null;
            mCursor = null;
            Log.e(TAG, "Encountered exception initializing cursor; " +
                  "cursor will not function.", e);
        }
    ...
    public int addWindow(Session session, IWindow client,
        ...
        synchronized(mWindowMap) {
            ...
            if (mDisplay == null) {
                ...
                START_CURSOR_X = mInitialDisplayWidth/2;
                START_CURSOR_Y = mInitialDisplayHeight/2;
            }
    ...
    private void ShowCursor(boolean b) {
        try {
            if (mCursor!=null && mShowCursor!=null && !mCursorVisible) {
                Object[] o = new Object[1];
                o[0] = Boolean.valueOf(b);
                mShowCursor.invoke(mCursor, o);
                MoveCursor(START_CURSOR_X, START_CURSOR_Y);
                mCursorVisible = true;
            }
        } catch (Exception e) {
            Log.e(TAG, "Exception in ShowCursor", e);
        }
    }

    private void MoveCursor(int x, int y) {
        try {
            int w = mInitialDisplayWidth;
            int h = mInitialDisplayHeight;
            if (mCursor!=null && mMoveCursor!=null) {
                Object[] o = new Object[5];
                WindowState top = (WindowState)mWindows.get(mWindows.size() - 1);
                o[0] = Integer.valueOf(x);
                o[1] = Integer.valueOf(y);
                o[2] = Integer.valueOf(w);
                o[3] = Integer.valueOf(h);
                o[4] = Integer.valueOf(top.mAnimLayer);
                mMoveCursor.invoke(mCursor, o);
            }
        } catch (Exception e) {
            Log.e(TAG, "Exception in MoveCursor", e);
        }
    }
                                ...
                                if ((mMlx != mcx || mMly != mcy)) {
                                    ...
                                    switch(mDisplay.getOrientation())
                                    {
                                        case Surface.ROTATION_0:
                                            MoveCursor(mMlx, mMly);
                                            break;
                                        case Surface.ROTATION_90:
                                            MoveCursor(mDisplay.getHeight() - mMly, mMlx);
                                            break;
                                        case Surface.ROTATION_180:
                                            MoveCursor(mDisplay.getWidth() - mMlx, mDisplay.getHeight()- mMly);
                                            break;
                                        case Surface.ROTATION_270:
                                            MoveCursor(mMly, mDisplay.getWidth()- mMlx);
                                            break;
                                    }
                                }
                                dispatchPointer(ev, (MotionEvent)ev.event, 0, 0);
