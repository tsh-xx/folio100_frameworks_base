...
{
#if !defined (NO_OPENCORE) || defined (USE_BOARD_MEDIARECORDER)
...
bool CheckForOggAudio(int fd, int64_t offset)
{
    bool bos = false;
    char buf[100];
    int noofsegments = 0;
    ...
    do {
        ...
        if (buf[5] & 0x02) {
            ...
            if (!strncmp("theora", buf + 1, 6)) {
                foundvideo = true;
                break;
            }

            lseek(fd, (datalength - 8), SEEK_CUR);
            memset(buf, 0, sizeof(buf));
            read(fd, buf, 27);
